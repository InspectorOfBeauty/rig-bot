...
public class CoefficientChangingConfirmationCallbackHandler implements RequestHandler {
    ...
    public boolean handle(UserRequest userRequest, UserSession userSession) {
        if (userSession != null && userSession.isAuthorized()
                && userSession.getLastUserAction().equals(UserActions.PRESSED_CHANGE_COEFFICIENT_BTN)
            ...
            userSessionRepository.updateLastUserActionByChatId(userRequest.getChatId(), UserActions.AGREED_CHANGE_COEFFICIENT_BTN);
            sender.sendMessage(String.valueOf(userRequest.getChatId()), Messages.INPUT_COEFFICIENT);
            return true;
        } else {
            return false;
        }
